...
class Argentina
	...
	specialRules: (originalNumber, withoutCountryCode, withoutNDC, ndc) =>
		if (ndc.length + withoutNDC.length) is 10
			...
			mobilePattern = new RegExp /^15/
			withoutMobile = withoutNDC.replace(mobilePattern, "")
			if withoutMobile.length < withoutNDC.length
				return (ndc.length + withoutMobile.length) is 10
			return false

	splitNumber: (number) =>
		switch number.length
			when 10
				splitNumber = number.split(/(\d{2})(\d{4})(\d{4})/)
				return _.filter splitNumber, (n) => n.length >= 1
			when 9
				splitNumber = number.split(/(\d{2})(\d{3})(\d{4})/)
				return _.filter splitNumber, (n) => n.length >= 1
			when 8
				if number.indexOf("15") is 0
					splitNumber = number.split(/(\d{2})(\d{2})(\d{4})/)
				else
...
window.vtex.phone.countries['54'] = new Argentina()